    ...
    def loss_computation_function(self):
            ...
            for condition_name in self.problem.conditions:
                ...
                if hasattr(condition, 'function'):
                    ...
                    pts_new = pts #pts[self.model.seq_length+1:,:].as_subclass(LabelTensor)
    ...
    def loss_computation_output(self):
            ...
            for condition_name in self.problem.conditions:
                ...
                if hasattr(condition, 'output_points'):
                    ...
                    residuals = (predicted - output_tensor ).reshape(-1,1)
                    local_loss = (
    ...
    def loss_computation(self):
            ...
            for condition_name in self.problem.conditions:
                ...
                if (condition_name == 'A'): 
                 ...
                 if (epoch == 0) or (epoch > 0 and epoch % 100 == 0): 
                  ...
                  if hasattr(condition, 'function'):
                    ...
                    pts_new.labels = self.model.input_variables                                        
                    for function in condition.function:
                        dydo_mult_o_Dis = function(self,pts_new, predicted)
                ...
                if hasattr(condition, 'output_points'):
                    ...
                    #residuals0 = (predicted[0,:] - output_tensor[0,:]).reshape(-1,1)
                    #local_loss_1 = 1*(
                        #condition.data_weight*self._compute_norm(residuals0)) 
                    #losses.append(local_loss_1) 
        ...
        while True:
            ...
            for condition_name in self.problem.conditions:
                ...
                if (condition_name == 'A'): 
                 ...
                 if (epoch == 0) or (epoch > 0 and epoch % 500 == 0): 
                  ...
                  if hasattr(condition, 'function'):
                    ...
                    for function in condition.function:
                        ...
                        #Derivative_current = Derivative['Residual_derivative'].detach()
                  ...
                  if hasattr(condition, 'function'):
                    ...
                    for function in condition.function:                                                
                        ...
                        for i in range(0,200-1,10):
                            ...
                            if i == 0: 
                              dmulty_Mat = torch.mm(Derivative_m1[k*400:(k+1)*400,:], predicted[i,:].reshape(-1,1)) #+ torch.mm(Derivative_current[k*400:(k+1)*400,:],predicted[i+1,:].reshape(-1,1))
                            else: 
                              dmulty_new = torch.mm(Derivative_m1[k*400:(k+1)*400,:], predicted[i,:].reshape(-1,1)) #+ torch.mm(Derivative_current[k*400:(k+1)*400,:],predicted[i+1,:].reshape(-1,1))
                ...
                if hasattr(condition, 'output_points'):
                    ...
                    output_tensor = condition.output_points                    
                    #tensors_y = torch.stack([output_tensor[i+self.model.seq_length] for i in range(len(output_tensor)-self.model.seq_length-1)])                    
                    #list_arrayD = np.load("list_arrayD_2.npy")
                    #list_arrayD = self.rand_choice_integer_Data 
                    #np.save("list_arrayD_100.npy",list_arrayD)
                    #residuals0 = (predicted[list_arrayD,:] - output_tensor[list_arrayD,:]).reshape(-1,1)
                    #list_arrayD = [0,50,100]
                    #residuals0 = (predicted[list_arrayD,:]- output_tensor[list_arrayD,:]).reshape(-1,1)
                    residuals0 = (predicted[0,:]- output_tensor[0,:]).reshape(-1,1)
                    local_loss_1 = 1*(
                        condition.data_weight*self._compute_norm(residuals0)) 
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            (losses[1]+losses[2]).backward(retain_graph=True)
            self.optimizer.step()
