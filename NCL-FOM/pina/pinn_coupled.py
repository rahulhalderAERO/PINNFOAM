            ...
            for condition_name in self.problem.conditions:
                  ...
                  if hasattr(condition, 'function'):
                    ...
                    for function in condition.function:                                                
                        ...
                        for i in range(0,200-1,10):
                            ...
                            if i == 0: 
                              dmulty_Mat = torch.mm(Derivative_m1[k*1200:(k+1)*1200,:], predicted[k,:].reshape(-1,1)) + torch.mm(Derivative_current[k*1200:(k+1)*1200,:],predicted[k+1,:].reshape(-1,1))
                            else: 
                              dmulty_new = torch.mm(Derivative_m1[k*1200:(k+1)*1200,:], predicted[k,:].reshape(-1,1)) + torch.mm(Derivative_current[k*1200:(k+1)*1200,:],predicted[k+1,:].reshape(-1,1))
                ...
                if (condition_name == 'D'):
                  ...
                  if hasattr(condition, 'function'):
                    ...
                    pts_new.labels = self.model.input_variables                       
                    for function in condition.function:                                                
                        y_mid = 10*function(self,pts_new, predicted)
